...
export function ImageBrowserView() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-background pt-[41px] pb-[50px]">
      {/* Top bar: search + update */}
      <div className="glass-bar glass-bar-edge-bottom px-4 py-2.5 flex gap-2 items-center shrink-0">
        <div className="relative max-w-xs flex-1">
          ...
          <Input
            ...
            className="pl-8 bg-background border-none rounded-lg text-[15px]"
          />
        </div>
        <Button variant="outline" size="sm" onClick={browser.executeSearch} className="rounded-lg border-none bg-background text-[15px]">
          Search
        </Button>
        ...
        <Button
          ...
          className="rounded-lg border-none bg-background text-[15px]"
        >
          {browser.updating ? (
        ...
        {browser.updateMessage && (
          <span className="text-[13px] text-muted-foreground max-w-xs truncate">
            {browser.updateMessage}
          </span>
        )}
      </div>
